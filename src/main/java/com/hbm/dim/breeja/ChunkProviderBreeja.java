...
public class ChunkProviderBreeja extends ChunkProviderCelestial {
	...
	public ChunkProviderBreeja(World world, long seed) {
		super(world, seed);
		caveGenV3.stoneBlock = ModBlocks.breeja_cobble;
		rgen.stoneBlock = ModBlocks.breeja_cobble;

		smallCrater.setSize(8, 32);
		largeCrater.setSize(96, 128);

		smallCrater.regolith = largeCrater.regolith = ModBlocks.basalt;
		smallCrater.rock = largeCrater.rock = ModBlocks.breeja_cobble;

		stoneBlock = ModBlocks.breeja_cobble;
	...
	protected Block getFlatWorldBlock(Block block) {
		...
		if(block == Blocks.grass || block == Blocks.sand) return ModBlocks.breeja_silt;
		if(block == Blocks.dirt || block == Blocks.stone || block == Blocks.sandstone) return ModBlocks.breeja_cobble;
