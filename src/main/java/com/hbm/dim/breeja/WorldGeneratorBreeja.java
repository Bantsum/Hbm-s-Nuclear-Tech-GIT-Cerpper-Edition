...
public class WorldGeneratorBreeja implements IWorldGenerator {

	public WorldGeneratorBreeja() {
		...
		NBTStructure.registerNullWeight(SpaceConfig.breejaDimension, 18);

		BlockOre.addValidBody(ModBlocks.ore_lithium, SolarSystem.Body.BREEJA);
		BlockOre.addValidBody(ModBlocks.ore_quartz, SolarSystem.Body.BREEJA);
		BlockOre.addValidBody(ModBlocks.ore_shale, SolarSystem.Body.BREEJA);

		BlockOre.addValidBody(ModBlocks.ore_brine, SolarSystem.Body.BREEJA);
	}

	@Override
	public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator, IChunkProvider chunkProvider) {
		if(world.provider.dimensionId == SpaceConfig.breejaDimension) {
			generateBreeja(world, random, chunkX * 16, chunkZ * 16);
		}
	}

	private void generateBreeja(World world, Random rand, int i, int j) {
